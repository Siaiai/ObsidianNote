...
flowchart TB
	...
	subgraph MID["中间件"]
		...
		MID_LED["LED"]
		APP_EM4095 --- MID_EM4095
		APP_READER --- MID_READER
		APP_PROTOCOL --- MID_PROTOCOL
		APP_LED --- MID_LED
	end
```
...
```mermaid
architecture-beta
    group main(cloud)["主循环"]
    group mid(server)["中间件"]

    service app_em4095(server)["app_em4095"] in main
    service app_reader(server)["app_reader"] in main
    service app_protocol(server)["app_protocol"] in main
    service app_led(server)["app_led"] in main

    service mid_em4095(server)["em4095"] in mid
    service mid_reader(server)["reader"] in mid
    service mid_protocol(server)["protocol"] in mid
    service mid_led(server)["LED"] in mid

    app_em4095:R -- L:app_reader
    app_reader:R -- L:app_protocol
    app_protocol:R -- L:app_led
    app_led:R -- L:app_em4095

    app_em4095:B -- T:mid_em4095
    app_em4095:B -- T:mid_led
    app_reader:B -- T:mid_reader
    app_protocol:B -- T:mid_protocol
    app_led:B -- T:mid_led
```
## 2. APP 胶水层
