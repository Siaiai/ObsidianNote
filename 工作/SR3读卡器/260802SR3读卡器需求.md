...
所有 UART 通信：19200-8-N-1。统一帧格式 `0x10 TYPE CMD DATA`，无 LEN 字段，无 BCC。帧长由 `CMD_TABLE` 的 `data_len` 定义。

### TYPE 分类

| TYPE | 含义 | 说明 |
|:----:|------|------|
| 0xFF | 卡号相关 | 查询/上报卡号/Fail/版本 |
| 0xA0 | 设置相关 | 复位等 |
| 0x0A | 升级相关 | 预留 |
...
| CMD | DATA | 完整字节序列 | 语义 | S2 | S3 | 实现 |
|:---:|:----:|------------|------|:--:|:--:|:----:|
| 0x7A | 无 | `10 FF 7A` | 进站查询 | ✅ | ✅ | ✅ |
| 0x75 | 无 | `10 FF 75` | 下放查询 | ✅ | ✅ | ✅ |
| 0x9A | 无 | `10 FF 9A` | 进站 + 切模式2 | ❌ | ❌ | ⬜ |
| 0x95 | 无 | `10 FF 95` | 下放 + 切模式2 | ❌ | ❌ | ⬜ |
| 0x9D | 无 | `10 FF 9D` | 切模式3 连续上报 | ❌ | ❌ | ⬜ |
| 0x90 | 无 | `10 FF 90` | 查版本 | ❌ | ❌ | ⬜ |
...
| CMD | DATA | 完整字节序列 | 语义 | S2 | S3 | 实现 |
|:---:|:----:|------------|------|:--:|:--:|:----:|
| 0x05 | ID[5] | `10 FF 05 AA BB CC DD EE` | 卡号包 | ✅ | ✅ | ✅ |
| 0x04 | `46 61 69 6C` | `10 FF 04 46 61 69 6C` | Fail包 | ❌ | ✅ | ✅ |
| 0x07 | `56 65 72 30 33 31 30` | `10 FF 07 56 65 72 30 33 31 30` | 版本包 "Ver0310" | ❌ | ❌ | ⬜ |
...
| CMD | DATA | 完整字节序列 | 语义 | S2 | S3 | 实现 |
|:---:|:----:|------------|------|:--:|:--:|:----:|
| 0x05 | 无 | `10 A0 05` | 复位（进Bootloader） | ❌ | ✅ | ✅ |
...
**实现状态：**
- ✅ 实现：3 条接收 + 2 条发送（S2/S3 实际使用的最小子集）
- ⬜ 不实现：5 条接收 + 1 条发送（文档写了但控制器没用，预留扩展）

**DATA 内容：**
- 卡号包 DATA = 5 字节卡号（大端）
- Fail包 DATA = ASCII "Fail"（`46 61 69 6C`，控制器只看前 3 字节 `10 FF 04` 判定 Fail 包，后面内容不检查）
- 版本包 DATA = ASCII "Ver0310"（7 字节）

**原 SR2/SR3 内容对比：**
- S2 控制器只发 `10 FF 7A`（进站）+ `10 FF 75`（下放），只收 `10 FF 05`（卡号包），无 Fail 包识别
- S3 控制器发 `10 FF 7A` + `10 FF 75` + `10 A0 05`（复位），收 `10 FF 05`（卡号包）+ `10 FF 04`（Fail 包）
- 两代控制器都不校验 BCC，都不用模式2/3/版本查询
