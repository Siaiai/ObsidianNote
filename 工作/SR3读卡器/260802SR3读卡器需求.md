...
flowchart TD
    ...
    Comp -->|"全1, 但写过失败"| BUG2["⚠️ 跳转损坏App<br/>变砖!"]
...
| 末包 Flash 写失败但 flag=1 | 完整性通过->**跳损坏App** | ⚠️ **变砖** |
...
```mermaid
flowchart LR
    subgraph 致命["🔴 致命缺陷 (会变砖)"]
        B1["Flash写失败仍回uOK<br/>+ 置flag=1<br/>-> 末包跳损坏App"]
        B2["Len>256 静默忽略<br/>-> 误触发末包完成"]
    end

    subgraph 卡死["🟠 卡死缺陷 (无看门狗)"]
        C1["IWDT 定义但从未调用<br/>任何卡死都不可自恢复"]
        C2["超时/写失败/栈非法<br/>-> 卡死等手动复位"]
        C3["GetPackFlag缺包 -> sFail<br/>不发错误, 卡死"]
    end
