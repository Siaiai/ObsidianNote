...
- **回复（B 发）**：B 收齐请求（重组到 rx_buf）调回调，回调返回 reply（指针+长度）。
  - `reply_len=0` 或没回调：纯 ACK（is_ACK=1, need_ACK=0, 无 data, 确认请求）
...
- **短回复合并**：回复单帧 `seq=请求seq`，一帧同时确认请求 + 带回复。长回复（分片）`seq=回复自己`，必须先纯 ACK 确认请求再发分片（seq 不冲突）。
...
sequenceDiagram
    ...
    Note over B: 没回调或回调无数据
    B-->>A: 纯ACK(确认请求)
    Note over A: A没reply回调, 完成
```
...
sequenceDiagram
    ...
    B-->>A: 纯ACK
    Note over A,B: B没回调或回调无数据, 末片纯ACK即确认
    Note over A: A没reply回调, 完成
```
