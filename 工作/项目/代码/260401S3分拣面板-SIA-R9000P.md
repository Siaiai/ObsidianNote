...
- **回复（B 发）**：B 收齐请求（重组到 rx_buf）调回调，回调返回 reply（指针+长度）。
  - `reply_len=0`（有回调无数据）：空回复帧（is_ACK=1, need_ACK=1, 无 data, A reply_callback(len=0)）
...
- **短回复合并**：回复单帧/空回复帧 `seq=请求seq`，一帧同时确认请求 + 带回复（或空回复）。长回复（分片）`seq=回复自己`，必须先纯 ACK 确认请求再发分片（seq 不冲突）。
