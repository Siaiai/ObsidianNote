...
2. 状态机：`IDLE -> SEND(发一片) -> WAIT_ACK(等该片纯ACK) -> 下一片 ... -> 末片纯ACK`。
3. 末片纯 ACK 收到（请求送达）：配了 reply_callback -> 进 `WAIT_REPLY` 等回复；没配 -> 完成。
4. `WAIT_REPLY`：收到回复帧（空/单/分片），库重组到 reply_buf，收齐 `reply_callback` -> IDLE；超时 `timeout_callback` -> IDLE。
5. 某片重传耗尽：跳过本包剩余片，`timeout_callback`。
...
sequenceDiagram
    ...
    participant B as 接收方B
    A->>B: 请求单帧(need_ACK=1, frag=11)
    B-->>A: 纯ACK(need_ACK=0, 确认请求)
    Note over A,B: A进WAIT_REPLY, B回调返长reply拆回复分片
...
sequenceDiagram
    ...
    B-->>A: 纯ACK(need_ACK=0)
    Note over A,B: B重组完回调返reply_len=0, A进WAIT_REPLY
    B->>A: 空回复帧(is_ACK=1, need_ACK=1, 无data)
    Note over A: reply_callback(len=0)
    A-->>B: 纯ACK(need_ACK=0)
```
...
sequenceDiagram
    ...
    A->>B: 请求片N(TAIL, need_ACK=1)
    B-->>A: 纯ACK(need_ACK=0)
    Note over A,B: B重组完回调返短reply, A进WAIT_REPLY
    B->>A: 回复单帧(is_ACK=1, need_ACK=1, 带reply)
    Note over A: reply_callback(data)
    A-->>B: 纯ACK(need_ACK=0)
```
...
sequenceDiagram
    ...
    A->>B: 请求片N(TAIL, need_ACK=1)
    B-->>A: 纯ACK(need_ACK=0)
    Note over A,B: B重组完回调返长reply拆回复分片, A进WAIT_REPLY
    B->>A: 回复片0(HEAD, is_ACK=1, need_ACK=1)
    A-->>B: 纯ACK(need_ACK=0)
    B->>A: 回复片M(TAIL, is_ACK=1, need_ACK=1)
    A-->>B: 纯ACK(need_ACK=0)
