...
sequenceDiagram
    ...
    Note over B: CmdCallback 交付数据(回复输出丢弃)
    Note over B: expect_ack=0, 不组回复, 直接IDLE
```
...
sequenceDiagram
    ...
    A->>B: 请求片1(重传, MIDDLE)
    B-->>A: 纯ACK(expect_ack=0)
    Note over A,B: 恢复; 重传耗尽(send_count>retry_max+1)则整包失败 timeout_callback
```
...
sequenceDiagram
    ...
    A->>B: 下个包片0(HEAD, expect_ack=1, seq>已见)
    Note over B: seq>rx_last_seq, 重置rx_active, 开始新包, 残包丢弃
    B-->>A: 纯ACK(expect_ack=0)
```
